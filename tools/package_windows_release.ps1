param(
    [string]$Version = "0.1.0-public-test.13",
...
- Восстановлена последовательность завершения миссии: предложение сохранения,
  подтверждение перезаписи занятого слота, FMV и переход к следующей миссии.
  Повтор ранее пройденного уровня больше не уменьшает прогресс кампании.
- Ручной прицел корректно отбирает управление у автоцели, сохраняет итоговое
  направление Гейба и поддерживает приседание без сдвига коллизионного корня.
- Исправлены очистка индикатора опасности и жизненный цикл выпавшего оружия:
  предмет следует за падающим противником и не исчезает между кадрами 20 Гц.
- В меню оружия восстановлены оригинальные пререндеренные арты MENU.HOG для
  всех типов оружия вместо затемнённых HUD-спрайтов и неверных подстановок.
- В метро восстановлены обе модели бомб и корректный банк их SPFX-текстур.
  Копии SCRIM публикуются атомарно, устраняя смешение текстур поезда и вывесок
  после перехода между игровыми сегментами.
- Вирусный сканер снова использует янтарный режим рентгена, показывает
  заражённые цели сквозь контейнеры и выводит оригинальные статусные строки.
- Динамические тени персонажей теперь включают оружие, устойчивее выбирают
  источник света и не теряются на переходах пола и стен.
- Letterbox оставлен только во вступлениях миссий; случайные полосы при
  прицеливании, радиопереговорах и обычных scripted-сценах удалены.
- Эмиссивные фонари и световые эффекты исключены из затемнения сцены;
  освещение объектов кэшируется на гостевом тике для стабильных 60–240 FPS.
- Переработаны аудиочасы и декодирование VAB: уменьшены задержки, пропуски
  сэмплов и заикания, восстановлен голосовой preview в меню громкости.
- В русской локали сохранены оригинальные английские подписи оптики,
  исправлены служебные сообщения и утечки кириллицы в английский интерфейс.
- Проведена полная проверка MSVC/PsyCross, ROM-проб и состава релизного архива.
