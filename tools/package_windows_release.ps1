param(
    [string]$Version = "0.1.0-public-test.11",
...
- Экспериментальное экранное контактное затенение (SSAO) полностью удалено
  вместе с шейдерами, промежуточными framebuffer и настройками лончера.
- Убраны синхронные запросы состояния OpenGL из горячих путей рендерера;
  состояние GPU явно кэшируется.
...
- Сохранены обычные динамические тени персонажей и оптимизации PT10.
