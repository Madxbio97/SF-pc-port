...
- Устранено исчерпание VRAM texture-alias pool: алиасы теперь живут по
  поколению сцены, а модели Гейба и противников не исчезают при стриминге.
- Исправлены выбор mission texture bank, перемещения SCRIM и наследование
  старых страниц VRAM между сегментами; добавлен Surface Picker с дампами.
- Переработан низкоуровневый SPU/OpenAL тракт и независимые CD/DMA часы:
  звук не зависит от частоты презентации до 240 FPS и не копит задержку.
- Добавлены динамический свет и геометрические тени персонажей с корректным
  переходом между полом и стенами, без выбеливания и двойного затемнения.
- Восстановлены оригинальные английские надписи прицелов; все подсказки с
  кнопками используют текущий PC-бинд, включая русское сообщение связи.
- Русские записи целей изолированы от английской локали и больше не могут
  подменять оригинальные строки гостевого меню.
- Дополнены русская локализация, шрифтовый атлас, игровые сообщения,
  брифинги, цели, параметры и двухстраничный раздел оружия.
- Восстановлены retail-читы через оригинальные комбинации и отдельную
  страницу Options > Cheats; launcher-side читы и выбор миссии удалены.
- Добавлены регрессионные тесты VRAM, аудиочасов, освещения, меню, ввода,
  локализации и потоковой загрузки миссий.
