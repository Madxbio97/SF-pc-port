param(
    [string]$Version = "0.1.0-public-test.10",
...
- Добавлен опциональный SSAO: мягкое контактное затенение мира в половинном
  разрешении с восстановлением reversed-Z и двусторонним размытием по глубине.
- Убраны синхронные запросы состояния OpenGL из кадрового, resolve- и SSAO-пути;
  состояние GPU теперь явно кэшируется рендерером.
- Полные обновления текстур VRAM заменены объединёнными загрузками изменённых
  строк, а нативный framebuffer преобразуется в гостевую VRAM без копии.
- Временные буферы света, теней, HUD, подписей и эффектов переиспользуются;
  планирование примитивов выполняется один раз на игровой тик 20 Гц, а не на
  каждый кадр презентации до 240 FPS.
- Снижена стоимость SSAO и фильтра без размытия границ геометрии.
