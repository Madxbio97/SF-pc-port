param(
    [string]$Version = "0.1.0-public-test.14",
...
- Восстановлен штатный экран провала сюжетных условий: плавное затемнение,
  посимвольное появление причины и единый безопасный путь перезапуска миссии.
- Добавлены корректные русские строки «УСЛОВИЕ МИССИИ НАРУШЕНО» и
  «ЦЕЛЬ МИССИИ НЕ ВЫПОЛНЕНА», включая незавершённую посимвольную анимацию.
- Доработано начало миссии: спокойное появление подсказки, короткий переход к
  управлению и корректное совмещение нативного и оригинального затемнения.
- Восстановлены размеры компактных предметов и плавная анимация подписей мира
  на частотах отображения от 30 до 240 FPS.
- Добавлен сглаженный счётчик реальной частоты кадров и времени кадра.
- Большой модуль PsyCross-сцены разделён на самостоятельные подсистемы;
  устранены повторные выделения памяти и неизменные вычисления рендера.
- Дополнены тесты экранов провала, локализации, HUD, аудио и R3000 runtime.
