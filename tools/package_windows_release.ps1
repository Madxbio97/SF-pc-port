param(
    [string]$Version = "0.1.0-public-test.12",
...
- Движение Гейба полностью блокируется на низком уровне до входа и во всё
  время прицеливания от первого лица; одновременные WASD и ПКМ больше не
  передают несовместимый корень коллизии между нативной и гостевой логикой.
- Вход в прицел изолирован от переката, восстановления и радиопереговоров;
  для повторной попытки после заблокированного перехода требуется отпустить
  кнопку прицела.
- Интерактивные рубильники, панели и другие обычные триггерные модели получают
  локальное освещение сцены вместо постоянной нейтральной яркости.
- Русские таблицы шрифтов пересобраны с безопасными для билинейной фильтрации
  границами ячеек и выровненными вертикальными метриками.
- Сохранены оптимизации рендерера PT10/PT11 и динамические тени персонажей.
- Проведена полная регрессионная проверка сборки и релизного пакета.
