...
constexpr std::array missions{
    ...
    MissionDefinition{2U, "Main subway line", "SUBWAY3", "SUBWAY3.OVL", "",
                      "EOL/SUBWAY3.STR", 2},
    MissionDefinition{3U, "Washington Park", "PARK", "PARK.OVL",
                      "SOL/PARK.STR", "", 3},
    ...
    MissionDefinition{8U, "Base Bunker", "BUNKER", "BASEEXT.OVL", "", "", 8, 1U},
    ...
    MissionDefinition{12U, "Stronghold lower level", "CHURCH2", "LEVSPEC.OVL",
                      "", "", 12, 1U},
    MissionDefinition{13U, "Stronghold catacombs", "CATACOMB",
                      "CATACOMB.OVL", "SOL/CATACOMB.STR",
                      "EOL/CATACOMB.STR", 13, 1U},
...
assets::HogArchive parseObjectModels(std::span<const std::byte> dlf) {
    ...
    if (archive_offset >= archive_end || archive_end > dlf.size()) {
        throw core::Error{core::ErrorCode::invalid_format, "DLF object-model archive is invalid"};
    }
    return assets::HogArchive::parse(copyBytes(dlf.subspan(archive_offset, archive_end - archive_offset)));
}
...
std::span<const std::string_view> missionScriptedMoviePaths(
...
MissionPackage::MissionPackage(
    ...
    : definition_(definition),
      ...
      has_retail_briefing_(has_retail_briefing),
      ...
      texture_banks_(std::move(texture_banks)),
      layout_(std::move(layout)),
      objects_(std::move(objects)),
      texture_file_count_(texture_file_count) {}

const assets::HogArchive& MissionPackage::textureBank(std::size_t bank) const {
    if (bank >= texture_banks_.size()) {
        throw core::Error{core::ErrorCode::invalid_argument, "Invalid mission texture bank"};
    }
...
MissionPackage MissionPackage::load(GameDisc& disc, std::uint32_t index) {
    ...
    auto archive = assets::FogArchive::parse(
        disc.image().readFile(archive_path));
    ...
    auto briefing = assets::MissionBriefing::fallback(
        std::string{definition.title});
    ...
    try {
        ...
        // Continuation levels contain gameplay DLF data without a briefing.
    }
    std::vector<assets::HogArchive> texture_banks;
    texture_banks.push_back(
        assets::HogArchive::parse(copyBytes(archive.file("VRAM.HOG"))));
    if (const auto bank = std::ranges::find_if(
            archive.entries(), [](const auto& entry) {
                return entry.name == "VRAM1.HOG";
            });
        bank != archive.entries().end()) {
        texture_banks.push_back(
            assets::HogArchive::parse(copyBytes(archive.file(bank->name))));
    }
    auto world_models = assets::HogArchive::parse(copyBytes(archive.file("WLDEMD.HOG")));
    auto object_models = parseObjectModels(archive.file("DLF.RFF"));
    auto special_effects = assets::HogArchive::parse(
        disc.image().readFile("COMMON/SPFX.HOG"));
    auto interface_assets = assets::HogArchive::parse(
        disc.image().readFile("COMMON/INTRFACE.HOG"));
    auto menu_assets = assets::HogArchive::parse(copyBytes(archive.file("MENU.HOG")));
    auto character_animations = assets::HogArchive::parse(
        disc.image().readFile("COMMON/PCHAN.HOG"));
    for (unsigned int frame = 0; frame < 8U; ++frame) {
        static_cast<void>(special_effects.file(
            "EXPL00" + std::to_string(frame) + ".TIM"));
    }
    ...
    auto objects = assets::MissionObjects::parse(
        archive.file(resource + ".BIN"));
    const auto texture_file_count = std::accumulate(
        texture_banks.begin(), texture_banks.end(), std::size_t{},
