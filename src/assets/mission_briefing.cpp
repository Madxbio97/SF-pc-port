...
std::vector<std::string> readBriefingStrings(std::span<const std::byte> dlf) {
    const auto data_offset = static_cast<std::size_t>(readLe32(dlf, data_offset_field));
    if (data_offset > dlf.size() || dlf.size() - data_offset < briefing_header_size) {
        throw core::Error{core::ErrorCode::invalid_format, "DLF briefing header is invalid"};
    }
    ...
    while (cursor < limit) {
        ...
        if (cursor > start && cursor < limit && dlf[cursor] == std::byte{}) {
            ...
            std::transform(
                dlf.begin() + static_cast<std::ptrdiff_t>(start),
...
MissionBriefing::MissionBriefing(
    ...
    : location_(std::move(location)),
      mission_title_(std::move(mission_title)),
      date_time_(std::move(date_time)),
...
MissionBriefing MissionBriefing::parseRecord(
    std::span<const std::byte> dlf,
    std::size_t record_index,
    std::string_view mission_title) {
    auto strings = readBriefingStrings(dlf);
    struct Record {
        std::size_t date{};
    };
    ...
    if (record_index >= records.size()) {
        throw core::Error{core::ErrorCode::invalid_format,
                          "DLF briefing record is missing"};
    }

    const auto date = records[record_index].date;
    auto date_time = strings[date];
    trimLineEnding(date_time);
    auto title = mission_title.empty() ? strings[date + 1U]
                                       : std::string{mission_title};
    // Only the first record stores the common geographic location. Retail
    // continuation records inherit it while advancing their own text/date.
