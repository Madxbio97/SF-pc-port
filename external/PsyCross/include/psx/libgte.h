...
#if USE_PGXP
/* Optional exact twins for PGXP. The packed MATRIX/SVECTOR values and every
 * legacy GTE result remain unchanged; these functions only preserve precision
 * which the PC renderer can use while building its vertices. */
extern void PGXP_MatrixRegister(const MATRIX* matrix, const double exact[9]);
extern int PGXP_MatrixLookup(const MATRIX* matrix, double exact[9]);
extern int PGXP_MatrixLookupCurrent(double exact[9]);
extern void PGXP_MatrixCopy(MATRIX* destination, const MATRIX* source);
extern void PGXP_MatrixCopyFull(MATRIX* destination, const MATRIX* source);
extern void PGXP_MatrixInvalidate(MATRIX* matrix);
extern void PGXP_MatrixRegisterTranslation(MATRIX* matrix, const double exact[3]);
extern void PGXP_MatrixRegisterTranslationQ12(MATRIX* matrix, int x, int y, int z);
extern int PGXP_MatrixLookupTranslation(const MATRIX* matrix, double exact[3]);
extern void PGXP_MatrixInvalidateTranslation(MATRIX* matrix);
extern void PGXP_MatrixNextGeneration(void);
extern void PGXP_VectorRegister(const SVECTOR* vector, const double exact[3]);
extern void PGXP_VectorRegisterQ12(const void* vector, int x, int y, int z);
extern void PGXP_VectorRegisterFixed(const void* vector, int x, int y, int z, int shift);
#endif
